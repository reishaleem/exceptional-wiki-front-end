...
export default () => {
  ...
  const onSubmit = (data) => {
    ...
    WikiService.createWiki(currentUser.id, universeId, wikiName, wikiBody).then(
      (response) => {
        setMessage(response.data.message);
        setSuccessful(true);
        history.push(`/app/universes/${universeId}`);
      },
      (error) => {
        const resMessage =
          (error.response &&
      ...
      <Container fluid style={{ paddingLeft: "0px" }}>
        ...
        <AppFrontPageWrapper>
          <Row style={{ paddingBottom: "15px" }}>
            <Col md={3}>
              <p>
                Maybe a picture for the thumbnail of the wiki, then underneath
                that, maybe the category thing. Not implementing the type yet,
                though.
              </p>
            </Col>
            <Col md={9}>
              <Form onSubmit={handleSubmit(onSubmit)}>
                <Form.Group controlId="formWikiName">
                  <Form.Label>What is your Wiki's name?</Form.Label>
                  <Form.Control
                    size="lg"
                    type="text"
                    name="wikiName"
